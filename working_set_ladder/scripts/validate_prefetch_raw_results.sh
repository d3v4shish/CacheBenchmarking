#!/usr/bin/env bash
# Independently validate the isolated prefetch-chain raw-event matrix.  These
# rows are deliberately not merged with the basic/branch timing ladder.

set -Eeuo pipefail

if [[ $# -ne 1 || ! -d "$1" ]]; then
  echo "usage: $0 /absolute/prefetch-raw-result-directory" >&2
  exit 64
fi
...
manifest="$run_root/manifest/result_sha256.txt"
...
grep -q 'prefetcht2' "$run_root/manifest/disassembly_with_opcodes.txt" || {
  echo "raw binary does not retain prefetcht2" >&2
  exit 65
}
...
awk -F, '
  ...
  END {
    ...
    exit bad
  }' "$samples" || { echo "invalid prefetch raw timing matrix" >&2; exit 65; }
...
awk -F, '
  ...
  END {
    ...
    if (NR - 1 < 336) bad = 1
    exit bad
  }' "$counters" || { echo "invalid prefetch raw PMU matrix" >&2; exit 65; }

echo "prefetch-chain raw validation passed: at least 6 accepted rows in every event/footprint cell"
