#!/usr/bin/env bash
# Reject incomplete, multiplexed, or checksum-mismatched MLP8 language rows.
set -Eeuo pipefail
...
awk -F, '
  ...
  echo "invalid MLP8 language sample matrix" >&2
  exit 65
}
[[ $(awk 'END { print NR - 1 }' "$raw/run_order.csv") == 420 ]] || {
  echo "wrong MLP8 language run order" >&2
  exit 65
}
echo "MLP8 language matrix validation passed"
