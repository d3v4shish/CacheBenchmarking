#!/usr/bin/env bash
# Four native front ends for one random 64-byte node chain.  Each iteration
# performs the same deliberately late PREFETCHT2 after `next` is revealed.
set -Eeuo pipefail
[[ $# == 1 && $1 == /home/d3v/prefetch-language-run-* && ! -e $1 ]] || { echo "usage: $0 /home/d3v/prefetch-language-run-UNIQUE" >&2; exit 64; }
run_root=$1
...
make -C "$root" BUILD_DIR="$run_root/bin" prefetch_language
for language in c cpp rust go; do
  objdump -d -Mintel --no-show-raw-insn "$run_root/bin/prefetch_$language" > "$run_root/manifest/prefetch_$language.disassembly.txt"
  grep -qi prefetcht2 "$run_root/manifest/prefetch_$language.disassembly.txt" || { echo "missing prefetcht2 in $language" >&2; exit 65; }
done
{ sha256sum "$root"/src/prefetch_{c.c,cpp.cpp,rust.rs} "$root"/src/prefetch_go/main.go "$root"/src/prefetch_go/prefetchasm/{prefetch.go,prefetch_amd64.s} "$root"/src/pmu_scope.c "$root"/src/pmu_scope.h "$root"/Makefile; find "$run_root/bin" -maxdepth 1 -type f -print0 | sort -z | xargs -0 sha256sum; } > "$run_root/manifest/source_binary_sha256.txt"
...
for round in $(seq 1 "$repetitions"); do
  ...
  while IFS=, read -r shuffle_key language footprint passes target; do
    ...
    output=$(taskset --cpu-list "$cpu" "$run_root/bin/prefetch_$language" "$footprint" "$passes")
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' "$sample_id" "$round" "$language" "$footprint" "$passes" "${fields[operations]}" "${fields[elapsed_ns]}" "${fields[checksum]}" "${fields[expected_checksum]}" "${fields[pmu_cycles]}" "${fields[pmu_instructions]}" "${fields[pmu_ref_cycles]}" "${fields[pmu_time_enabled]}" "${fields[pmu_time_running]}" "${fields[pmu_available]}" >> "$run_root/raw/samples.csv"
  done < <(shuffle_cases "$round")
done
"$root/scripts/validate_prefetch_language_results.sh" "$run_root/raw"
"$root/scripts/analyze_prefetch_language_results.sh" "$run_root/raw"
