#!/usr/bin/env bash
# Same MLP8 C++ source under four compiler/vectorization policies.  This is a
# code-generation control; lane count stays eight and no variant is SIMD.
set -Eeuo pipefail

[[ $# == 1 && $1 == /home/d3v/mlp8-cpp-compilers-run-* && ! -e $1 ]] || {
  echo "usage: $0 /home/d3v/mlp8-cpp-compilers-run-UNIQUE" >&2
  exit 64
}
run_root=$1
...
for tool in gcc-13 g++-13 clang++-18 ar objdump; do
...
common=(-std=c++20 -O3 -march=haswell -mtune=haswell -mavx2 -DMLP_LANES=8
  -Wall -Wextra -Werror -I"$root/src" "$root/src/mlp8_cpp.cpp" "$run_root/bin/libscanpmu.a")
...
printf '%q ' g++-13 "${common[@]}" -fno-tree-vectorize -fno-tree-slp-vectorize >"$run_root/manifest/build_commands.txt"
printf '\n%q ' g++-13 "${common[@]}" >>"$run_root/manifest/build_commands.txt"
printf '\n%q ' clang++-18 "${common[@]}" -fno-vectorize -fno-slp-vectorize >>"$run_root/manifest/build_commands.txt"
printf '\n%q ' clang++-18 "${common[@]}" >>"$run_root/manifest/build_commands.txt"
...
for spec in '32768 450000000' '36864 450000000' '262144 350000000' '1048576 250000000' '6291456 120000000' '8388608 120000000' '536870912 24000000'; do
...
"$root/scripts/validate_mlp8_cpp_compilers_results.sh" "$run_root/raw"
"$root/scripts/analyze_mlp8_cpp_compilers_results.sh" "$run_root/raw"
