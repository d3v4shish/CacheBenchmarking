...
raw=$1
# `asort` is a gawk extension and is unavailable on the DUT's default awk.
# Sort each measured ratio first, then select the eighth of fifteen retained
# repetitions.  This is the same portable median protocol as other controls.
awk -F, 'NR>1 { print $3 "," $4 "," $7/$6 "," $10/$6 "," $11/$6 "," $12/$6 }' "$raw/samples.csv" |
  ...
  awk -F, 'BEGIN { print "language,footprint_bytes,repetitions,median_ns_per_operation,p10_ns_per_operation,p90_ns_per_operation,median_cycles_per_operation,p10_cycles_per_operation,p90_cycles_per_operation,median_instructions_per_operation,median_ref_cycles_per_operation" }
    ...
    END { emit() }' > "$raw/summary_arena_language.csv"
[[ $(wc -l < "$raw/summary_arena_language.csv") == 29 ]] || { echo "wrong summary rows" >&2; exit 65; }
